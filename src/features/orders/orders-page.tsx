...
export function OrdersPage(){
  ...
  return <>
    ...
    {query.isPending?<LoadingState label="正在加载用料记录…"/>:query.isError?<ErrorState error={query.error} retry={()=>void query.refetch()}/>:<section className="overflow-hidden rounded-[14px] border border-black/10 bg-white">{query.data.items.length?<div className="overflow-auto"><table className="w-full min-w-[1240px] border-collapse text-xs [&_th]:border-b [&_th]:border-black/10 [&_th]:bg-[#fafaf8] [&_th]:px-4 [&_th]:py-3 [&_th]:text-left [&_th]:font-medium [&_th]:text-[#77756e] [&_td]:border-b [&_td]:border-black/5 [&_td]:px-4 [&_td]:py-3.5"><thead><tr><th>业务日期</th><th>单号</th><th>油漆工</th><th>工地</th><th>材料</th><th>原用料金额</th><th>退料</th><th>净用料金额</th><th>已结金额</th><th>未结金额</th><th>状态</th><th>操作人</th><th/></tr></thead><tbody>{query.data.items.map(item=>{const net=fromMinorUnits(toMinorUnits(item.finalAmount)-toMinorUnits(item.returnedAmount));return <tr className="hover:bg-[#fafaf8]" key={item.id}><td className="whitespace-nowrap font-medium tabular-nums text-[#22211f]">{new Date(item.occurredAt).toLocaleDateString('zh-CN')}</td><td><Link className="font-medium text-[#22211f] underline-offset-4 hover:underline" href={`/orders/${item.id}`}>{item.orderNo}</Link></td><td>{item.workerName}</td><td>{item.projectName||'未指定'}</td><td>{item.items.length} 项</td><td className="tabular-nums">{formatMoney(item.finalAmount)}</td><td className="tabular-nums text-[#558063]">-{formatMoney(item.returnedAmount)}</td><td className="font-medium tabular-nums">{formatMoney(net)}</td><td className="tabular-nums text-[#558063]">{formatMoney(item.settledAmount)}</td><td className="font-medium tabular-nums text-[#a34e40]">{formatMoney(item.outstandingAmount)}</td><td><span className="rounded-md bg-[#efeeeb] px-2 py-1 text-[10px]">{statusLabels[item.status]}</span></td><td>{item.operatorName}</td><td><Button asChild className="h-8 px-2.5" variant="ghost"><Link href={`/orders/${item.id}`}>查看详情</Link></Button></td></tr>;})}</tbody></table></div>:<EmptyState title="暂无用料记录" description="创建第一张用料单后会显示在这里。"/>}<Pagination page={page} pageSize={query.data.pageSize} total={query.data.total} href={next=>{const value=new URLSearchParams(params);value.set('page',String(next));return `${pathname}?${value}`;}}/></section>}
  </>;
}
