...
import { findMockOrder } from './order-service';
...
function filter<T extends {workerId:string;workerName:string;createdAt:string}>(items:T[],request:NextRequest){const q=request.nextUrl.searchParams;const search=(q.get('search')||'').toLowerCase();const workerId=q.get('workerId');let next=[...items];if(search)next=next.filter((item)=>item.workerName.toLowerCase().includes(search));if(workerId)next=next.filter((item)=>item.workerId===workerId);return next;}
